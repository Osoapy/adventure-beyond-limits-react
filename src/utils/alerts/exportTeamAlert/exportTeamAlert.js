...
const treatingText = (text) => {
    let formatedText = text.replaceAll("-", " ").toLowerCase();
...
const exportTeamAlert = (pokemonTeam) => {
    ...
    for (let i = 0; i < pokemonTeam.length; i++) {
        ...
        if (p.heldItem && p.heldItem.trim() !== "") {
            showDownText += `@ ${treatingText(p.heldItem)}`;
        }

        showDownText += `\n`;

        // ABILITY
        if (p.ability) showDownText += `Ability: ${treatingText(p.ability)}\n`;
        ...
        // TERA TYPE
        if (p.teraType) showDownText += `Tera Type: ${treatingText(p.teraType)}\n`;
        ...
        if (p.moves && Array.isArray(p.moves)) {
            for (let move of p.moves) {
                if (move === "" || move === null || move === undefined) continue;
                showDownText += `- ${treatingText(move)}\n`;
            }
        }
...
export default exportTeamAlert;
