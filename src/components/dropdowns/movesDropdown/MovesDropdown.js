...
export default function MovesDropdown({ species, onSelect, initialValue }) {
  ...
  return (
    <div
      ref={containerRef}
      className={styles['moves-dropdown'] + ' ' + styles['move'] + ' ' + styles[selectedType]}
      style={{ position: 'relative', width: '100%' }}
    >
      <input
        className={styles['input-moves-dropdown'] + ' ' + styles[selectedType]}
        ref={inputRef}
        value={search}
        ...
        >
          {filtered.map(name => {
            ...
            return (
              <li
                key={name}
                className={styles['dropdown-item'] + ' ' + styles[type]}
                onClick={() => {
                  setSearch(name);
